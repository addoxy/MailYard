...
// Derived atoms
export const hasActiveSearchAtom = atom((get) => get(searchQueryAtom).trim() !== '');

export const isDefaultViewAtom = atom((get) => {
  const searchQuery = get(searchQueryAtom);
  const activeFilter = get(activeFilterAtom);
  const sortOrder = get(sortOrderAtom);

  return searchQuery === '' && activeFilter === 'all' && sortOrder === 'modified-desc';
});
...
export const toggleDesignSelectionAtom = atom(null, (get, set, designId: string) => {
  const currentSelection = get(selectedDesignIdsAtom);

  if (currentSelection.includes(designId)) {
    set(
      selectedDesignIdsAtom,
      currentSelection.filter((id) => id !== designId)
    );
  } else {
...
export const selectAllDesignsAtom = atom(null, (get, set, designIds: string[]) => {
